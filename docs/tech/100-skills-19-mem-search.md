---
title: "100 个 Skills 第 19 期：mem-search"
description: "Search claude-mem's persistent cross-session memory database"
tags: [OpenClaw, Skill, 自动化]
readingTime: 5
date: 2026-04-20
---

# 100 个 Skills 第 19 期：mem-search - Search claude-mem's persistent cross-session memory database. Use when user asks "did we already solve this?", "how did we do X last time?", or needs work from previous sessions.

今天介绍的 skill 是 **mem-search**。

它的核心定位很直接：**Search claude-mem's persistent cross-session memory database. Use when user asks "did we already solve this?", "how did we do X last time?", or needs work from previous sessions.**

## 基本信息

| 项目 | 内容 |
|------|------|
| Skill 名称 | mem-search |
| 来源目录 | `/Users/frankyuan/.agents/skills/mem-search` |
| 风险等级 | 🟢 LOW ~ 🟡 MEDIUM（需结合实际能力判断） |

## 适合解决什么问题

- 提高日常效率
- 让 agent 更方便调用特定能力
- 减少重复性操作

## 使用示例

```bash
# 暂无示例
```

## 我的看法

这个 skill 的价值在于：

1. **定位清晰**：功能边界比较明确
2. **接入成本低**：从文档示例看，上手难度不高
3. **适合进入 workflow**：可以作为更大自动化流程中的一个能力模块

## 使用建议

如果你准备把它纳入日常工作流，建议：

- 先手动跑一遍核心示例
- 再决定是否接进自动化流程
- 如果涉及外部写操作，先做小范围验证

---

**系列进度**：19/100
